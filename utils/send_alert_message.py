...
def _send_wecom_any(title, ts_code, chart_period=None):
    if _send_channel('企业微信群机器人', send_wecom_webhook_message, title, ts_code, chart_period=chart_period):
        return
    _send_channel('企业微信应用消息', send_wecom_message, title, ts_code, chart_period=chart_period)
...
def send_alert_message(title, ts_code, chart_period=None):
    ...
    if channel in {'both', 'all', 'dingtalk_wecom', 'dingtalk+wecom', '钉钉企业微信'}:
        ...
        _send_wecom_any(title, ts_code, chart_period=chart_period)
        return
